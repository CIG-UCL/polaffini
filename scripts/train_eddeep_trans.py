...
parser.add_argument('-t', '--train_data', type=str, required=True, help='Path to the training data following nested subfolder structure below.')
parser.add_argument('-v', '--val_data', type=str, required=False, default=None, help='Path to the validation data following nested subfolder structure below.')
# ├── sub1
# │   ├── ped1
# │   │   ├── bval1
...
# │   │   │   ├── ...
# │   │   ├── ...
# │   │   ├── sub1_ped1_bval1_meandir.nii.gz (only required for the taget bval)
# │   │   ├── sub1_ped1_bval2_meandir.nii.gz (only required for the taget bval)
# │   │   ├── ...
# │   └── ped2
# │       ├── ...
# ├── sub2
...
parser.add_argument('-gdec', '--gen-dec-nf', type=int, nargs='+', required=False, default=[256,128,64,32], help="Number of decoder featuresfor the generator. Default: 256 128 64 32.")
...
os.makedirs(os.path.join(os.path.dirname(args.model), 'imgs'), exist_ok=True)
...
gen_train = generators.DWtoDWmean(subdirs=sub_dirs,
...
if not is_val:  
    ...
    gen_val = generators.DWtoDWmean(subdirs=sub_dirs_val,                                                          
                                    k=len(args.gen_enc_nf),
                                    target_bval = args.target_bval,
                                    get_dwmean = True,
                                    spat_aug_prob = 0,
...
if args.resume:
    # load existing model
    discriminator = dwarp.networks.pix2pix_dis.load(dis_last_path)
    generator = dwarp.networks.pix2pix_gen.load(gen_last_path)
    tab_loss = pandas.read_csv(loss_file, sep=',')
    best_img_loss = np.min(tab_loss.val_img0 + tab_loss.val_img0) / 2
    initial_epoch = tab_loss.epoch.iloc[-1]
    print('resuming training at epoch: ' + str(initial_epoch))

else:
    # build the models
    discriminator = dwarp.networks.pix2pix_dis(inshape,
                                               nb_feats=args.dis_enc_nf)   
    generator = dwarp.networks.pix2pix_gen(volshape=inshape,
...
gan = dwarp.networks.gan(generator, discriminator, inshape)
...
val_steps_per_epoch = n_val // args.batch_size
...
for epoch in range(initial_epoch, args.epochs):
    ...
    dis0_real_loss_epoch = np.mean(dis0_real_loss_epoch); val_dis0_real_loss_epoch = np.mean(val_dis0_real_loss_epoch) if gen_val is not None else np.nan
    dis0_fake_loss_epoch = np.mean(dis0_fake_loss_epoch); val_dis0_fake_loss_epoch = np.mean(val_dis0_fake_loss_epoch) if gen_val is not None else np.nan
    gen0_loss_epoch = np.mean(gen0_loss_epoch);           val_gen0_loss_epoch = np.mean(val_gen0_loss_epoch) if gen_val is not None else np.nan
    img0_loss_epoch = np.mean(img0_loss_epoch);           val_img0_loss_epoch = np.mean(val_img0_loss_epoch) if gen_val is not None else np.nan
    dis_real_loss_epoch = np.mean(dis_real_loss_epoch);   val_dis_real_loss_epoch = np.mean(val_dis_real_loss_epoch) if gen_val is not None else np.nan
    dis_fake_loss_epoch = np.mean(dis_fake_loss_epoch);   val_dis_fake_loss_epoch = np.mean(val_dis_fake_loss_epoch) if gen_val is not None else np.nan
    gen_loss_epoch = np.mean(gen_loss_epoch);             val_gen_loss_epoch = np.mean(val_gen_loss_epoch) if gen_val is not None else np.nan
    img_loss_epoch = np.mean(img_loss_epoch);             val_img_loss_epoch = np.mean(val_img_loss_epoch) if gen_val is not None else np.nan
    ...
    plt.savefig(os.path.join(os.path.dirname(args.model), 'imgs','img_' + str(epoch) + '.png'), bbox_inches='tight')
    plt.close()
