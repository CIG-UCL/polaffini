...
parser.add_argument('-ms', '--mov-seg', type=pathlib.Path, required=False, default=None, help='Path to the moving segmentation (only required for POLAFFINI initialization). Must be a directory if -m is also a directory, where segmentations are stored in the same alphabetical order as the images.')
